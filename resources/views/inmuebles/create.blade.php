<x-layout>
    ...
    <x-card class="p-10 max-w-lg mx-auto mt-24">
        ...
        <form method="POST" action="/inmuebles">
            @csrf
            <div class="mb-6">
                ...
                <input type="text" class="border border-gray-200 rounded p-2 w-full" name="titulo"
                    placeholder="Minimo 10 caracteres" value="{{ old('titulo') }}" />
                @error('titulo')
            ...
            <div class="mb-6">
                ...
                <input type="text" class="border border-gray-200 rounded p-2 w-full" name="precio"
                    placeholder="Solo datos numericos" value="{{ old('precio') }}" />
                @error('precio')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="text" class="border border-gray-200 rounded p-2 w-full" name="ubicacion"
                    placeholder="Ejemplo: Av Artesanos #1448, Guadalajara" value="{{ old('ubicacion') }}" />
                @error('ubicacion')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <input type="text" class="border border-gray-200 rounded p-2 w-full" name="contacto"
                    placeholder="Dirección Email válida" value="{{ old('contacto') }}" />
                @error('contacto')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                @enderror
            </div>
